...
from prometheus_agent.config_schema import AgentConfig, LlmConfig, CognitiveToolkitConfig
...
class TestCognitiveGovernor(unittest.IsolatedAsyncioTestCase):
    """
    ...
    def setUp(self):
        ...
        self.mock_agent = MagicMock(name="MockAgent")

        # Define a mock configuration, mimicking config.json using Pydantic models
        self.mock_config = AgentConfig(
            ...
            cognitive_toolkit=CognitiveToolkitConfig(
                skill_routing={
                    "evaluator_judge": "fast_model",
                    "knowledge_transmutation": "power_model"
                },
                skill_timeouts={ "default": 10, "some_skill": 20 }
            )
        )
        self.mock_agent.config = self.mock_config
        self.mock_agent.model_orchestrator = None # Disable for these tests
        self.mock_agent.scm = None # Disable for these tests
    ...
    async def test_01_initialization_and_alias_map_creation(self):
    ...
    async def test_02_initialization_failure_with_incomplete_config(self):
        ...
        # Mock a config with missing aliases
        incomplete_config = AgentConfig(llm=LlmConfig(aliases={"default_model": "phi3"}))
        incomplete_config_agent.config = incomplete_config
        incomplete_config_agent.skill_map = {}

        with self.assertRaisesRegex(ValueError, "is missing one of the required keys"):
            CognitiveGovernor(agent_instance=incomplete_config_agent)

    async def test_03_precedence_1_domain_specific_skill_model(self):
        """Tests that a model defined for a specific skill in its domain is chosen first."""
        # --- Act ---
        decision = await self.governor.decide_model(skill="manifold_code_gen", domain="coding")
    ...
    async def test_04_precedence_2_global_skill_override_model(self):
        ...
        # --- Act ---
        decision = await self.governor.decide_model(skill="knowledge_transmutation", domain="some_other_domain")
    ...
    async def test_05_precedence_3_domain_default_model(self):
        ...
        # --- Act ---
        decision = await self.governor.decide_model(skill="knowledge_placement", domain="metacognition")
    ...
    async def test_06_precedence_4_global_default_model(self):
        ...
        # --- Act ---
        decision = await self.governor.decide_model(skill="completely_unknown_skill", domain="non_existent_domain")

        # --- Assert ---
        # Should resolve the global 'default_model' alias
        self.assertEqual(decision.model_name, "phi3:latest")
        self.assertIn("Global Default Fallback", decision.reason)

    async def test_07_resolve_alias_logic(self):
